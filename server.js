...
app.get('/api/items', (req, res) => {
  ...
  // Si no se pasa parámetro q, devolver todos los productos
  if (!query) {
      return res.json(products);
  }

  // Si se pasa parámetro q, filtrar los productos por el título
...
app.post('/api/addSale', (req, res) => {
    ...
    if (product) {
        const sale = { id: sales.length + 1, productTitle: product.title, date: new Date(), totalPrice: product.price };
        sales.push(sale);
